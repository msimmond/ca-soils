---
title: "![](images/logo.png) Soil Health Assessment Project </span><br> Regional Region"
subtitle: "Producer Report"
params:
  producer_id: null
  year: null
  grouping_var: null
...
# DEBUG: log every read.csv and utils::read.csv call
if (interactive() || TRUE) {
  # mask unqualified calls
  read.csv <- function(file, ...) {
    message("DEBUG read.csv -> ", normalizePath(file, winslash = "/", mustWork = FALSE))
    utils::read.csv(file, ...)
  }
  # trace namespace-qualified calls
  if (requireNamespace("utils", quietly = TRUE)) {
    try(trace(utils::read.csv,
              quote(message("TRACE utils::read.csv -> ", normalizePath(file, winslash = "/", mustWork = FALSE))),
...
if (file_ext %in% c("xlsx", "xls")) {
  ...
  message("Using data sheet: ", data_sheet)
...
if (file_ext %in% c("xlsx", "xls")) {
  ...
  if ("Data Dictionary" %in% sheets) {
    ...
    message("Loaded dictionary from 'Data Dictionary' sheet")
  } else {
  ...
  if (!is.null(params$dict_path) && file.exists(params$dict_path)) {
    ...
    if (is.null(dictionary)) {
      message("Dictionary attribute not found on data; falling back to file…")
...
cat("Debug - Measurement columns to pivot:", paste(measurement_cols, collapse = ", "), "\n")
...
if (nrow(producer_samples) == 0) {
  cat("Debug - No producer samples found. Available producers:", unique(results_long$producer_id), "\n")
  cat("Debug - Looking for:", params$producer_id, "\n")
  cat("Debug - Available years:", unique(results_long$year), "\n")
  cat("Debug - Looking for year:", params$year, "\n")
...
cat("Debug - grouping_var value:", params$grouping_var, "\n")
cat("Debug - grouping_var class:", class(params$grouping_var), "\n")
cat("Debug - grouping_var in names:", params$grouping_var %in% names(producer_samples), "\n")
...
# Debug: print a tiny sample right before plotting
if (exists("agg")) {
  print(head(agg, 10))
  print(table(agg$category, agg$plotting_group, useNA="ifany"))
}
```

```{r make-texture-triangle, fig.cap="Soil texture triangle", out.width="100%", fig.align="center", message=TRUE}
ragg::agg_png(file.path(path, "texture-triangle.png"), res = 150, width = 1500, height = 1000, scaling = 1)
...
# Which producer groups exist this year (values of the chosen grouping var)
...
# Symbol/color maps reused across figures
...
# 1) All Other fields points (now one point per other producer+year+grouping var)
if (!is.null(texture_list$`Other Fields`)) {
  .add_pts(
    texture_list$`Other Fields`,
    col = base::paste0(as.character(other_color), "CC"),
    bg  = base::paste0(as.character(other_color), "99"),
    pch = 21,
    size = 1.36
  )
}

# 2) Your fields, one symbol/color per producer group value
if (!is.null(texture_list$`Your Fields`)) {
  yf <- texture_list$`Your Fields`
  if (length(producer_groups)) {
    for (g in names(sym_map)) {
      .add_pts(
        dplyr::filter(yf, plotting_group == g),           # <-- matches treatment_id, etc.
        col = base::paste0(col_map[g], "FF"),
        bg  = base::paste0(col_map[g], "CC"),
        pch = sym_map[g],
        size = 2.4
      )
    }
    ...
    .add_pts(yf,
      col = base::paste0(as.character(primary_color), "FF"),
      bg  = base::paste0(as.character(primary_color), "CC"),
      pch = 16, size = 2.4
    )
  }
}

# Legend (producer groups first, then one "Other Fields")
if (length(producer_groups)) {
  casoils::add_legend(
    legend = c(names(sym_map), "Other Fields"),
    color  = c(unname(col_map), as.character(other_color)),
    pt.bg  = c(base::paste0(unname(col_map), "CC"),
               base::paste0(as.character(other_color), "99")),
  ...
  casoils::add_legend(
    ...
    pt.bg  = c(base::paste0(as.character(primary_color), "CC"),
               base::paste0(as.character(other_color), "99")),
    pch    = c(16, 21),
    size   = c(2.4, 1.36),
    vertical_spacing = 1.5
...
if (requireNamespace("magick", quietly = TRUE)) {
  tt <- magick::image_read(file.path(path, "texture-triangle.png"))
  tt <- magick::image_trim(tt)
  magick::image_write(tt, file.path(path, "texture-triangle.png"))
}

# Add these 3 lines so the figure actually appears in the doc
texture_png <- file.path(path, "texture-triangle.png")
if (file.exists(texture_png)) {
  knitr::include_graphics(texture_png)
}
```
...
For the project we took soil samples from your selected fields using the grouping variable "{{ params$grouping_var }}". You can see the GPS locations of your fields above. The other data included in this report are from other fields in the project for comparison.

<!-- VMW Note that you'll need to change "orchards" to "annual fields" for Sarah's sites-->

The results below are organized into tables and graphs describing the physical, chemical, carbon, and biological measurements from your soils. Each point represents an average of the five samples we collected from each field. All samples that were sent to the lab were collected from 0-6 inches depth, while in-field soil measurements were collected from a 6-8 inch hole. ***All in-field indicators are marked with an asterisk (*).***
